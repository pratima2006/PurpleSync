...
const helps = [
  { q: "App samajh nahi aa raha?", a: "Topbar me 'Take a Tour' pe click karo. 5 steps me pura app ka tour ho jayega - Home, Voting, Schedule sab cover hota hai." },
  { q: "Link kaam nahi kar raha?", a: "Official sources kabhi-kabhi region lock hote hai. Try karo VPN ya dusra browser. Phir bhi na khule to Help me report karo." },
  { q: "Koi bug mila to kya karu?", a: "Screenshot ke saath GitHub pe issue banao ya Help page pe detail likho. Hum jaldi fix kar denge." },
];
export function Help() {
  ...
  return (
    <div className="relative min-h-[80vh] overflow-hidden p-6 md:p-10">
      ...
      <h1 className="text-[22px] font-semibold text-[#2f2640]">Help</h1>
        ...
        {helps.map((h,i) => {
          ...
          return (
            <button key={i} onClick={()=>setOpen(isOpen?null:i)} className={`w-full text-left rounded-[12px] px-5 py-4 transition-all ${isOpen?'bg-white shadow-[0_10px_30px_rgba(96,67,143,0.12)]':'bg-[#e9e2ff]/70 backdrop-blur-md'}`}>
              ...
              <p className="mt-2 text-[14px] font-semibold text-[#3d2d5a]">{h.q}</p>
              {isOpen && <p className="mt-3 text-[13px] leading-6 text-[#6e6582]">{h.a}</p>}
            </button>
          )
        })}
      </div>
