...
export function Achievements() {
  ...
  if(view==='archive'){
    return (
      <div className="ps-page-enter ps-content py-8 md:py-12">
        <style>{`.font-santrio{font-family:'Santrio',serif;}`}</style>
        ...
        <div className="flex items-center justify-between">
          <h1 className="font-santrio text-[28px] text-[#332840] font-normal tracking-[-.02em]">Full archive.</h1>
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {archiveList.map((item:any)=>(
            <article key={item.id||item.title} onClick={()=>{setSelected(item); setView('detail')}} className="ps-panel ps-panel-hover rounded-2xl p-5 cursor-pointer">
              ...
              <h3 className="font-santrio mt-8 text-[15px] font-normal leading-[1.3] text-[#40334e]">{item.title}</h3>
  ...
  return (
    <div className="ps-page-enter ps-content py-8 md:py-12">
      ...
      <div>
        ...
        <h1 className="font-santrio mt-2 text-[40px] leading-[.9] tracking-[-.03em] text-[#332840] font-normal">A body of<br /><em className="text-[#704ca5] not-italic">work.</em></h1>
      ...
      <div className="mt-10 grid gap-4 md:grid-cols-[1.2fr_.8fr]">
        <div className="relative overflow-hidden rounded-2xl bg-[#60438f] p-7 text-white md:p-9">
          ...
          <h2 className="font-santrio relative mt-8 max-w-[500px] text-[26px] leading-[1.08] md:text-[32px] font-normal tracking-[-.01em]">{latest?.title?.split('.').slice(0,2).join('.') || "Three decades.\nOne name at the centre."}</h2>
