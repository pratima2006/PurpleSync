...
function formatTwoUnits(item: any) {
...
export function Voting() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "votingItems"), (snap) => {
      ...
      if (firestoreItems.length > 0) {
        setAllItems([...firestoreItems,...defaultVotingItems]);
      } else {
  ...
  const closest = useMemo(() => {
    const openWithDate = allItems.filter((i: any) => i.status === 'open' && getTargetTime(i));
    if (openWithDate.length > 0) {
      openWithDate.sort((a: any, b: any) => (getTargetTime(a) || 0) - (getTargetTime(b) || 0));
      return openWithDate[0];
    }
    return allItems.find((i: any) => i.status === 'open') || allItems[0];
  }, [allItems, tick]);
  ...
  const countdown = useMemo(() => {
    if (closest && getTargetTime(closest)) {
      return formatFourUnits(closest);
    }
    return `${String(Math.floor(seconds / 3600)).padStart(2, '0')}h ${String(
      Math.floor((seconds % 3600) / 60),
    ).padStart(2, '0')}m ${String(seconds % 60).padStart(2, '0')}s`;
  }, [closest, seconds, tick]);
      ...
      <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
        ...
        <div className="rounded-2xl border border-[#e0d4ed] bg-[#f1e9f9] p-4 md:min-w-[225px]">
          ...
          <p className="mt-1 text-[10px] text-[#887b99] truncate">
            {closest? `${closest.title} · ${closest.platform}` : 'Global Fan Choice · Mnet Plus'}
          </p>
        </div>
      </div>
